...
public class AddToCartService extends OrderCalculateService implements ShoppingCart {
	...
	public void addItem(final String upc, final Double price, final int quantity) {
		...
		Optional<Entry<String,ItemVO>> optional = this.order.getOrderLines().entrySet().stream().filter(i->i.getKey().equals(upc)).findFirst();
		if(optional.isPresent()) {
			ItemVO existingItem = this.order.getOrderLines().get(upc);
			existingItem.setQuantity(existingItem.getQuantity()+quantity);
			this.order.getOrderLines().put(upc, existingItem);
			return;
		}
	...
	public void caclulate() {
		super.calculateOrder(this.order);
	}
